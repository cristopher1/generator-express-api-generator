...
Based on [generator-koa2-api-generator](https://github.com/cristopher1/generator-koa2-api-generator)

The project generated by generator-express-api-generator use ES Modules and async/await functions.

### What's changed? See [generator-express-api-generator releases](https://github.com/cristopher1/generator-express-api-generator/releases)

Example of a generator created by `generator-express-api-generator`:
...
First, install [Yeoman](http://yeoman.io) and generator-express-api-generator using [npm](https://www.npmjs.com/) (we assume you have pre-installed [node.js](https://nodejs.org/)).

```bash
npm install -g yo
npm install -g generator-express-api-generator
```

Then generate your new project:

```bash
yo express-api-generator project_name
```
...
```console
PS C:\Users\...\new_express_api_project> yo express-api-generator project_name
```

## <a id="arguments-and-options"></a> Arguments and options

The generator-express-api-generator include two arguments called **projectName** and **databaseDriver** (postgresql or mysql, or mariadb). **projectName** is a required argument:

For example: If you want to create a new express api called express_api_project (projectName = express_api_project) with postgresql plugin, you should use:

```bash
yo express-api-generator express_api_project postgresql
```

The generator-express-api-generator include various options, these are:

| option            |  value  | default | description                                                                                                   | example                                                                         |
| :---------------- | :-----: | :-----: | :------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------ |
| runGitInit        | Boolean |  false  | Run git init automatically, then installing the dependencies                                                  | `yo express-api-generator project_name --runGitInit`                            |
| runPackageScripts | Boolean |  false  | Run the scripts that configure the package, then installing the dependencies                                  | `yo express-api-generator project_name --runPackageScripts`                     |
| useDocker         | Boolean |  false  | Add docker support using DockerFile, .dockerignore and others                                                 | `yo express-api-generator project_name --useDocker`                             |
| nodeVersion       | Number  |   16    | Node version used in DockerFile. (FROM nodeVersion). Recommended to use node 16, 18, 20 or 21                 | `yo express-api-generator project_name --nodeVersion=21`                        |
| projectFolderName | String  |   api   | Project folder name used in DockerFile. (WORKDIR /usr/src/projectFolderName)                                  | `yo express-api-generator project_name --projectFolderName=project_folder_name` |
| useDockerCompose  | String  |  false  | Add Docker Compose support.                                                                                   | `yo express-api-generator project_name --useDockerCompose`                      |
| databaseName      | String  |  null   | Select the database to which the application will connect. Accepts the values: postgresql, mysql and mariadb. | `yo express-api-generator project_name --databaseName=postgresql`               |

## <a id="structure"></a> Project structure

The project generated by generator-express-api-generator includes:
...
- **api (folder)**: In this folder there are project configurations and the src folder that contains the source code
  ...
  - **README.md (file)**: Documentation generated by generator-express-api-generator.
  - **src (folder)**: Contains the source code:
    ...
    - **db (folder)**: Contains folders used by sequelize.
      ...
      - **models (folder)**: This folder contains ES Modules (by default the project generated by generator-express-api-generator uses ES Modules, files with extension .js and use import/export). Includes a default user model.
    ...
    - **routes (folder)**: Contains the Express routers used by the project.
      ...
      - **index.js (file)**: Contains the main Express router with prefix 'api/v1' and mounts the others routers.
    - **schemas (folder)**: Contains openapi schemas and json schemas.
      ...
      - **openapi (folder)**: Contains openapi schemas. The files in this folder have the .yml extension and they can have any name, for example: 'my_new_openapi_schema.yml'.
        - **default_response (folder)**: Contains openapi schemas for Express default response (200, 201, 401, 404, 500).
    ...
    - **types.d.ts (file)**: Includes any types used by the project, for example type Orm, JsonSchema, Module augmentation (Express), etc. You can call the Orm/JsonSchema/others type using jsdoc, for example: /\*_ @type {import('route/to/typesfolder/types.d.ts').Orm} _/
...
`express-api-generator:app` used by `yo express-api-generator`: It is the generator used by default.

`express-api-generator:docker`: It is used to add support for Docker. It accepts the options `--useDocker`, `--nodeVersion` and `--projectFolderName`.

`express-api-generator:docker_compose`: It is used to add support for Docker Compose. It accepts the options `--useDockerCompose` and `--databaseName`.
